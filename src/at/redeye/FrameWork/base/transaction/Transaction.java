...
public abstract class Transaction {
	...
	/**
	 * Options controlling behaviour of {@code fetchTable2} and related methods.
	 * Pass as varargs parameters, e.g. {@code fetchTable2(binddesc, where, EXCLUDE_BLOBS)}.
	 */
	public enum FetchOption {
		/** Exclude BLOB columns from the SELECT so binary data is not transferred. */
		EXCLUDE_BLOBS
	}
	// end AI generated
	...
	public <T extends DBStrukt> List<T> fetchTable2(T binddesc, String where)
			throws SQLException, TableBindingNotRegisteredException,
			UnsupportedDBDataTypeException, WrongBindFileFormatException {
		return fetchTable2Internal(binddesc, where, null, false);
	}

	// generated by AI (GitHub Copilot)
	/**
	 * Like {@link #fetchTable2(DBStrukt, String)} but with {@link FetchOption}
	 * flags, e.g. {@code fetchTable2(binddesc, where, EXCLUDE_BLOBS)}.
	 */
	public <T extends DBStrukt> List<T> fetchTable2(T binddesc, String where,
			FetchOption... options)
			throws SQLException, TableBindingNotRegisteredException,
			UnsupportedDBDataTypeException, WrongBindFileFormatException {
		boolean excludeBlobs = shouldExcludeBlobs(options);
		return fetchTable2Internal(binddesc, where, null, excludeBlobs);
	}
	// end AI generated

	private <T extends DBStrukt> List<T> fetchTable2Internal(T binddesc,
			String where, List<Object> params, boolean excludeBlobs)
			throws SQLException, TableBindingNotRegisteredException,
			UnsupportedDBDataTypeException, WrongBindFileFormatException {
		...
		if (params != null) {
			result = executer.fetchTableValue(tablenames, where, params, excludeBlobs);
		} else {
			result = executer.fetchTableValue(tablenames, where, null, excludeBlobs);
		}
	 ...
	 * @param options  optional {@link FetchOption} flags, e.g. {@code EXCLUDE_BLOBS}
	...
	public <T extends DBStrukt> List<T> fetchTable2(T strukt, Condition condition,
		...
		return fetchTable2Internal(strukt, whereSql, params, excludeBlobs);
	}

	// generated by AI (GitHub Copilot)
	private static boolean shouldExcludeBlobs(FetchOption... options) {
		if (options != null) {
			for (FetchOption opt : options) {
				if (opt == FetchOption.EXCLUDE_BLOBS) {
					return true;
				}
			}
		}
