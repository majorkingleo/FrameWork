...
public class StringUtils {
	...
	public static String autoLineBreak(String what, int length) {
		...
		final int searchWindowLength = 20;

		if (length < searchWindowLength / 2 || length >= what.length()
				|| searchWindowLength / 2 >= what.length()) {
			// Doesn't make sense
			return what;
		}
		...
		for (int walker = 1; walker < in.length; walker++) {
			...
			if (in.length <= searchWindowLength / 2) {
				break;
			}
			...
			if (walker % length == 0) {
				...
				for (int signidx = 0; signidx < myPreferedSigns.length; signidx++) {
					...
					for (int index = 1; index <= (searchWindowLength / 2); index++) {
