...
public class DerbyTransaction extends Transaction {
	...
	protected DerbyTransaction() {

	}

	public DerbyTransaction(ConnectionDefinition def)
	...
	private String getPeriodExclStmt(String column, String str_begin,
			String str_end) {
		...
		if (fromFilter != null && fromFilter.size() > 0) {
			...
			for (int i = 0; i < fromFilter.size(); i++) {
				...
				if (c instanceof DBFilterComboBox) {
					...
					valueFrom = ((DBFilterComboBox) c).getSelectedItem()
							.toString();
					...
					// not supported item
					System.out
							.println("Unsupported filter element in container!");
					return "";
				...
				for (int j = 0; j < toFilter.size(); j++) {
					...
					if (c instanceof DBFilterComboBox) {
						...
						valueTo = ((DBFilterComboBox) cTo).getSelectedItem()
								.toString();
					} else if (c instanceof DBFilterEditField) {
						nameTo = ((DBFilterComboBox) cTo).getColName();
						valueTo = ((DBFilterComboBox) cTo).getSelectedItem()
