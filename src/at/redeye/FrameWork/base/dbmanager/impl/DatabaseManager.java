...
public class DatabaseManager implements DBManager, DBBindtypeManager {
	...
	public boolean migrateTable(DBStrukt strukt, Integer fromVersion)
		...
		// Drop all FK constraints before touching the table so ALTER TABLE
		// column additions are not blocked by referential checks.
		dropAllForeignKeys(strukt);
	...
	/**
	 * Executes DROP CONSTRAINT (or dialect equivalent) for every FK declared on
	 * {@code strukt}. Errors are silently logged: the constraint may not yet exist
	 * on a fresh install or on a DBMS that does not support FK constraints.
	 *
