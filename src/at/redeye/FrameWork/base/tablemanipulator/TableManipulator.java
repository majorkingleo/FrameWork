...
public class TableManipulator {
  ...
  */
    ...
    private void installTypedSortComparators()
    {
        ...
        TableRowSorter<NormalTableModel> sorter = null;

        if( table.getRowSorter() instanceof TableRowSorter &&
            table.getRowSorter().getModel() == model )
        {
            sorter = (TableRowSorter<NormalTableModel>) table.getRowSorter();
        }
        else
        {
            sorter = new TableRowSorter<NormalTableModel>(model);
            table.setRowSorter(sorter);
        }

        for( int i = 0; i < model.getColumnCount(); i++ )
        {
            ColumnSortType type = getSortTypeForColumn(i);

            if( type == ColumnSortType.NUMERIC )
                sorter.setComparator(i, getNumericComparator());
            else if( type == ColumnSortType.DATE )
                sorter.setComparator(i, getDateComparator());
            else
                sorter.setComparator(i, getTextComparator());
        }
    }

    // generated by AI (GitHub Copilot / GPT-5.3-Codex)
    private ColumnSortType getSortTypeForColumn( int modelColumn )
    {
        if( tabledesign == null ||
            tabledesign.colls == null ||
            modelColumn < 0 ||
            modelColumn >= tabledesign.colls.size() )
        {
            return ColumnSortType.TEXT;
        }

        TableDesign.Coll coll = tabledesign.colls.get(modelColumn);

        if( coll.dbval == null )
            return ColumnSortType.TEXT;

        DBDataType type = coll.dbval.getDBType();

        if( type == DBDataType.DB_TYPE_INTEGER ||
            type == DBDataType.DB_TYPE_LONG ||
            type == DBDataType.DB_TYPE_SHORT ||
            type == DBDataType.DB_TYPE_DOUBLE ||
            type == DBDataType.DB_TYPE_FLOAT )
        {
            return ColumnSortType.NUMERIC;
        }

        if( type == DBDataType.DB_TYPE_DATE ||
            type == DBDataType.DB_TYPE_TIME ||
            type == DBDataType.DB_TYPE_DATETIME )
        {
            return ColumnSortType.DATE;
        }
    ...
    // generated by AI (GitHub Copilot / GPT-5.3-Codex)
    private Comparator<Object> getNumericComparator()
    {
    ...
    // generated by AI (GitHub Copilot / GPT-5.3-Codex)
    private Comparator<Object> getDateComparator()
    {
        return new Comparator<Object>() {
            ...
            public int compare(Object o1, Object o2)
            {
                Long t1 = toTimestamp(o1);
                Long t2 = toTimestamp(o2);
    ...
    // generated by AI (GitHub Copilot / GPT-5.3-Codex)
    private Comparator<Object> getTextComparator()
    {
        return new Comparator<Object>() {
            ...
            public int compare(Object o1, Object o2)
            {
                String s1 = toComparableText(o1);
                String s2 = toComparableText(o2);
    ...
    // generated by AI (GitHub Copilot / GPT-5.3-Codex)
    private String toComparableText(Object value)
    {
        if( value == null )
            return "";
    ...
    private Double toDouble(Object value)
    {
        ...
        String txt = normalized.toString();
    ...
    private Long toTimestamp(Object value)
    {
        ...
        if( normalized instanceof Date )
            return ((Date)normalized).getTime();
