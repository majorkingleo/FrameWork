...
import at.redeye.FrameWork.base.bindtypes.Condition;
...
public abstract class Transaction {
	...
	/**
	 * Fetches rows matching a type-safe {@link Condition} built from
	 * {@code static final DBValue} column references.
	 *
	 * <p>Values are bound via JDBC {@code ?} parameters — never interpolated
	 * into the SQL string.</p>
	 *
	 * Generated by AI (GitHub Copilot / Claude Sonnet 4.6)
	 *
	 * @param strukt template for the target table
	 * @param condition WHERE condition built with {@link Condition#where(DBValue)}
	 * @return list of matching rows, possibly empty
	 */
	public <T extends DBStrukt> List<T> fetchTable2(T strukt, Condition condition)
		...
		String whereSql = condition.renderWhere(
				executer.getStmtCreator(), strukt.getName());
		...
		List<T> res = new ArrayList<T>(result.size());
		for (int i = 0; i < result.size(); i++) {
			DBStrukt row = strukt.getNewOne();
			row.consumeFast(result.get(i));
			res.add((T) row);
		}
		return res;
	}
	...
	public <T extends DBStrukt> List<T> fetchChildren(
		...
		Object refRawVal = refVal != null ? refVal.getValue() : null;
		DBValue ownerCol = childStrukt.getValue(fk.getOwnerColumn());
		if (ownerCol == null) {
			throw new ForeignKeyNotFoundException(
				"Column '" + fk.getOwnerColumn() + "' not found in strukt "
				+ childStrukt.getName());
		}
		return fetchTable2(childStrukt, Condition.where(ownerCol).eq(refRawVal));
	}
	...
	public <T extends DBStrukt> List<T> fetchChildren(
		...
		return fetchTable2(childStrukt, Condition.where(ownerCol).eq(fkValue));
	}
	...
	public <P extends DBStrukt> P fetchParent(
		...
		DBValue fkCol = childStrukt.getValue(fkColumnName);
		Object rawVal = fkCol != null ? fkCol.getValue() : null;
		DBValue refCol = parentStrukt.getValue(fk.getReferencedColumn());
		if (refCol == null) {
			throw new ForeignKeyNotFoundException(
				"Column '" + fk.getReferencedColumn() + "' not found in parent strukt "
				+ parentStrukt.getName());
		}
		List<P> result = fetchTable2(parentStrukt, Condition.where(refCol).eq(rawVal));
