...
public class LeastTransparentImage {
	...
	public void addAll(List<DBImage> images) {
	...
	private MinMax getMinMaxTransparentImage_int() {
		...
		for (Content entry : images) {
			...
			PixelGrabber grabber = new PixelGrabber(entry.img, 0, 0, width,
			...
			for (int x = 0, y = 0; x < width && y < height; x++, y++) {
				...
				/*
				 * System.out.println(String.format(entry.image.file_name.getValue
				 * () + " %X alpha %X", val, alpha));
				 */
