...
public interface StmtExecInterface {
	...
	/**
	 * Executes a SELECT with a parameterised WHERE clause using PreparedStatement
	 * bind variables.  The {@code whereStmt} must contain exactly as many
	 * {@code ?} placeholders as entries in {@code params}.
	 *
	 * Generated by AI (GitHub Copilot / Claude Sonnet 4.6)
	 *
	 * @param tablenames table names to select from
	 * @param whereStmt  WHERE clause with {@code ?} placeholders
	 ...
	 * excludes BLOB columns from the SELECT so binary data is not transferred.
	 *
	 * @param tablenames   table names to select from
	 * @param whereStmt    WHERE clause (can include {@code ?} placeholders)
	 * @param params       bind values for placeholders; may be null or empty
	 * @param excludeBlobs if true, BLOB columns are omitted from SELECT and results
	 * @return selected rows (without BLOB values if excluded)
	 */
	public List<HashMap<String, Object>> fetchTableValue(String[] tablenames,
