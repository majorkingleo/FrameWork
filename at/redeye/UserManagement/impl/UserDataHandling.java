...
public class UserDataHandling implements UserManagementInterface {
	...
	public DBPb checkUserData(String login, String pwd,
		...
		List<DBStrukt> pbrecords = getAllUserData();
		String encPwd = null;
		DBPb currpb = null;
		...
		if (pbrecords.size() > 0) {
			...
			if (currpb != null && found == true && autoLoginRequested == true) {
	...
	public boolean tryAutoLogin() {
		...
		String strAutoLogin = root.getSetup().getConfig(
				FrameWorkConfigDefinitions.AllowAutoLogin);
		boolean autoLoginAllowed = Boolean.parseBoolean(strAutoLogin);
		if (autoLoginAllowed != true) {
			return false;
		}

		String autoLoginUser = root.getSetup().getLocalConfig(
				FrameWorkConfigDefinitions.AutoLoginUser);
