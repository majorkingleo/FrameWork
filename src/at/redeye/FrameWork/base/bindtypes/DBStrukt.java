...
public abstract class DBStrukt {
	...
	/** Versioned list of FK constraints declared on this table. Generated by AI (GitHub Copilot / Claude Sonnet 4.6) */
	protected ArrayList<Entry<Integer, ForeignKeyDefinition>> foreign_keys_ = new ArrayList<Entry<Integer, ForeignKeyDefinition>>();
	...
	// -----------------------------------------------------------------------
	// Foreign key support  Generated by AI (GitHub Copilot / Claude Sonnet 4.6)
	// -----------------------------------------------------------------------

	/**
	 * Register a foreign key constraint present since schema version 1.
	 *
	 * @param fk the FK definition; if {@code name} is {@code null} a name is
	 *           auto-generated as {@code FK_<TABLE>_<COLUMN>} (uppercased).
	 */
	...
	/**
	 * Register a foreign key constraint introduced at the given schema version.
	 * Bumps the table version the same way {@link #add(DBValue, Integer)} does.
	 *
	 * @param fk      the FK definition; name is auto-generated if {@code null}.
	 * @param version schema version at which this FK was introduced
	 */
	public void addForeignKey(ForeignKeyDefinition fk, int version) {
		if (fk.getName() == null) {
			String autoName = "FK_"
				+ strukt_name.toUpperCase().replaceAll("[^A-Z0-9]", "_")
				+ "_"
				+ fk.getOwnerColumn().toUpperCase().replaceAll("[^A-Z0-9]", "_");
			fk = fk.withName(autoName);
		}
		foreign_keys_.add(new SimpleEntry<Integer, ForeignKeyDefinition>(version, fk));
	...
	/**
	 * Returns all FK constraints registered on this table, in declaration order.
	 *
	 * @return list of {@link ForeignKeyDefinition}; empty when no FKs are declared
	 */
	public ArrayList<ForeignKeyDefinition> getForeignKeys() {
		...
		for (Entry<Integer, ForeignKeyDefinition> entry : foreign_keys_) {
	...
	/**
	 * Returns FK constraints introduced at exactly the given schema version.
	 * Used during migration to apply only the FKs added at a specific step.
	 *
	 * @param version the schema version to filter by
	 * @return list of matching {@link ForeignKeyDefinition}; may be empty
	 */
	public ArrayList<ForeignKeyDefinition> getForeignKeysForVersion(int version) {
		ArrayList<ForeignKeyDefinition> result = new ArrayList<ForeignKeyDefinition>();
		for (Entry<Integer, ForeignKeyDefinition> entry : foreign_keys_) {
	...
	/**
	 * Returns FK constraints introduced at schema version &lt;= {@code maxVersion}.
	 * Used during migration to drop only the FKs that actually exist in the
	 * current (old) database schema before running ALTER TABLE statements.
	 *
	 ...
	 * @return list of matching {@link ForeignKeyDefinition}; may be empty
	 */
	public ArrayList<ForeignKeyDefinition> getForeignKeysUpToVersion(int maxVersion) {
		ArrayList<ForeignKeyDefinition> result = new ArrayList<ForeignKeyDefinition>();
		for (Entry<Integer, ForeignKeyDefinition> entry : foreign_keys_) {
