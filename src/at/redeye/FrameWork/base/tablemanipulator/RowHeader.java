...
public class RowHeader
{
    ...
    {
        // Keep row header exactly in sync with JTable row height to avoid drifting.
        int table_row_height = table != null ? table.getRowHeight() : height;

        if( table_row_height <= 0 )
            table_row_height = height;

        if( table_row_height <= 0 )
            table_row_height = 1;

        if( list != null )
            list.setFixedCellHeight(table_row_height);
    }
    // AI modification end
